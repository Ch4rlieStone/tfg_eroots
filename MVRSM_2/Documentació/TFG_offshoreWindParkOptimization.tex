...
\begin{tikzpicture}[node distance=2cm, auto]
    ...
    \draw [->] (box8) -- ++(5,0) node[midway, above] {YES}|-  node[pos=0.25,sloped] {Optimization algorithm: NSGA-II [\ref{NSGAII}]} (box2);
...
The tuning parameters used for the NSGA-II are shown in Table \ref{tab:NSGAII_parameters}.
...
\begin{itemize}
    ...
    \item OOPF dona subport a que les solucions de NSGA-II son correctes.
    \item Proposta innovadora de fer lapproach al problema mixed variables.
    \item Amb mateix temps de computació, i menys, trobem tot un set de optimal solutions.
\end{itemize}
\subsection{Future work}\label{futurework}
\begin{itemize}
    \item Desenvolupar bé el tema dels diferents winds. Proposta de com fer-ho i això permetria comparar resultats directament amb state-of-art.
    \item Mirar afectarien varaicions de voltage level a al main grid we supply to.
    \item Fer més casos d'estudi.
    \item Implementar eel mètode a un software com podria ser Gridcal.
\end{itemize}
%------------------------------------------
