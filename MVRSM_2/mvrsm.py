...
class SurrogateModel:
    ...
    def g(self, x): #Change this to have the objective index in the argument
        """
        Evaluates the surrogate model at `x`.
        :param x: the decision variable values.
        """
        phi = self.phi(x)
        return np.inner(self.c, phi) # c[obj_index}]
...
def MVRSM_minimize(obj, x0, lb, ub, num_int, max_evals, rand_evals=0):
    ...
    log_filename = f'data/log_MVRSM_{str(start_time)}.log'
    ...
    for i in range(0, max_evals):
        ...
        y_unscaled, cost_inv, cost_tech = obj(x)
        cost_inv_vec.append(cost_inv)
        cost_tech_vec.append(cost_tech)
        if i == 0:
            y0 = y_unscaled
    ...
    return best_x, inv_scale(best_y, y0), model, log_filename, cost_inv_vec, cost_tech_vec
...
def visualise_model(model, obj, x0, lb, ub, num_int):
    ...
    for i in range(len(int_range)):
        ...
        for j in range(len(cont_range)):
            ...
            obj_output[i, j], cost_inv, cost_tech = obj(x)
