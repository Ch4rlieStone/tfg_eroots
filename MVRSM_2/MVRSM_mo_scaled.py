...
def MVRSM_mo_scaled(obj_func,
    ...
    for i in range(rand_evals):
        ...
        next_x = np.zeros(d) # Initial guess
        next_x[0:num_int] = np.round(np.random.rand(num_int)*(ub[0:num_int]-lb[0:num_int]) + lb[0:num_int]) # Random initial guess (integer)
        next_x[num_int:d] = np.random.rand(d-num_int)*(ub[num_int:d]-lb[num_int:d]) + lb[num_int:d] # Random initial guess (continuous)
